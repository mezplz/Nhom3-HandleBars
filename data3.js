...
function Display(element){
    for (let i = 0 ; i < categories.length ; i++){
        if (element.textContent == categories[i].title){
            cate = categories[i].id
        }
    }
}
...
if (cate > 0){
    for (let i = 0 ; i < products.length ; i++){
        if (products[i].category == cate){
            productTemplate.push(products[i])
        }
    }
}
else {
...
var templateText = $("#task3-content").html();
var templateScript = Handlebars.compile(templateText);
var html = templateScript({ productTemplate: productTemplate });
$("#task3-content-target").html(html);
});